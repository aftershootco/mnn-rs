...
mod tensor_v1 {
    ...
    impl<'a, TT> TensorRef<'a, TT> {
        ...
        pub fn create_host_tensor_from_device(&self, copy_data: bool) -> Tensor<Host> {
            let tensor =
                unsafe { Tensor_createHostTensorFromDevice(self.tensor, copy_data as i32) };
    ...
    impl<T: TensorType> Clone for Tensor<T> {
        fn clone(&self) -> Self {
            let tensor_ptr = unsafe { Tensor_clone(self.tensor.tensor) };
            let tensor = unsafe { TensorRef::from_ptr(tensor_ptr) };
            Self {
                tensor,
                __marker: PhantomData,
            }
